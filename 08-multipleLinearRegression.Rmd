...
\noindent where each $b_k$ estimates its corresponding parameter $\beta_k$.
...
```{r Figure8-4,fig.cap="(ref:fig8-4)",fig.width=8,fig.height=4.5}
...
\noindent but the observed snow depth was actually $y_9=11.2$ inches. The observed  ***residual*** is then
...
```{r Figure8-6,fig.cap="(ref:fig8-6)",fig.height=4.5, fig.width=8}
...
```{r Figure8-7,fig.cap="(ref:fig8-7)",fig.height=5,fig.width=8}
par(mfrow=c(2,2), oma=c(0,0,2,0))
...
```{r Figure8-8,fig.cap="(ref:fig8-8)",fig.height=5,fig.width=8}
...
\noindent is used, but we need to have "something" to put in for the two temperature
...
\noindent which incorporates the *degrees of freedom* for the model via the error 
...
\noindent given that *Max.Temp* is in the model.
...
\noindent looks like it might be worth interrogating further so we can jump straight
...
```{r Figure8-21,fig.cap="(ref:fig8-21)",fig.height=4,fig.width=7,echo=F}
require(heplots)
data(Headache)
scatterplot(du2~du1|treatment, data=Headache, smooth=F, lwd=2,
            ...
            legend.coords="topleft",cex.main=1,cex.leg=0.75)
```
...
\noindent where the $I_{\text{CatName}J,i}\text{'s}$ are the different indicator variables. 
...
```{r Figure8-22,fig.cap="(ref:fig8-22)",echo=F}
...
\noindent where the new component involves both the indicator and the quantitative
...
\noindent and simplified for the two groups as:
...
\noindent and so provides a value of 0 for the top-ranked AIC model and a measure of how
...
\noindent based on the estimated coefficients provided below. Using these results and the
