...
\indent An example of snow depths at some high elevation locations on a day in 
...
then the slope would have been $0.0163*1000=16.3$ inches change in mean
...
```{r Figure8-3a,fig.cap="(ref:fig8-3a)",echo=F,fig.height=3,fig.width=5}
...
```{r Figure8-3b,fig.cap="(ref:fig8-3b)",echo=F,fig.height=3,fig.width=5}
...
```{r Figure8-4,fig.cap="(ref:fig8-4)",fig.width=10,fig.height=5}
...
* **Independence of observations:**
    ...
    * The observations are based on a random sample of sites from the 
    ...
    observations (this is called ***spatial correlation***). I have
...
```{r Figure8-5,fig.cap="(ref:fig8-5)",fig.height=5,fig.width=8}
...
\indent The estimated snow depth for "Combination" is </span>
...
\indent The Residuals vs. Leverage diagnostic plot in Figure \@ref(fig:Figure8-7) 
...
```{r Figure8-8,fig.cap="(ref:fig8-8)",fig.height=5,fig.width=10,echo=F}
...
\indent The term-plots (Figure \@ref(fig:Figure8-9)) show that the temperature slopes
...
```{r Figure8-9,fig.cap="(ref:fig8-9)",fig.height=4.5,fig.width=8}
...
```{r Figure8-10,fig.cap="(ref:fig8-10)",echo=F}
par(mfrow=c(1,1))
...
\indent To get the associated 95% CIs, we could return to using the ``predict``
...
* **Be cautious about doing prediction in MLR -- you might be doing  extrapolation!**
    ...
    ```{r Figure8-11,fig.cap="Scatterplot of observed Elevations and Maximum Temperatures for SNOTEL data.",echo=F,fig.width=5,fig.height=3}
...
* **Multicollinearity in MLR models:**
    ...
    ```{r Figure8-12,fig.cap="Plot of correlation matrix in the snow depth data set with influential points removed",fig.width=6.5,echo=F}
    require(corrplot)
    par(mfrow=c(1,1), oma=c(0,0,1,0))
    corrplot.mixed(cor(snotel2[-c(9,22),3:6]), col=c("black", "orange"),
                   tl.cex=0.75)
...
There is strong evidence to suggest that the true mean 
...
\indent Remember when testing slope coefficients in MLR, that if we fail to 
...
coefficient might be located, given the other variables in the model, and so are usually pretty interesting to report. 
...
```{r Figure8-13,fig.cap="(ref:fig8-13)",fig.height=5,fig.width=7}
require(openintro)
data(satGPA)
require(tibble)
...
```{r Figure8-14,fig.cap="(ref:fig8-14)",fig.height=5,fig.width=8}
...
2. Validity conditions:
    ...
    * **Quantitative variables condition:**
        ...
        * The variables used here in this model are quantitative. Note that *sex* was
    ...
    * **Linearity of relationships:**
        ...
        * The Residuals vs Fitted and Scale-Location plots (Figure 
        ...
        which is our desired result.
            ...
            * Together, there is no evidence of a violation of the linearity
            assumption.
    ...
    * **Equal (constant) variance:**
        ...
        * There is no clear change in variability as a function of fitted 
        values so no evidence of a violation of the constant variance of residuals assumption.
...
6. Conclusions and Scope of Inference:
    ...
    * For *SATV*: There is strong evidence to reject the null hypothesis of 
    no linear relationship between *SATV* and *FYGPA* ($t_997=8.88$, p-value<0.0001) and conclude that, in 
    ...
    * For *SATM*: There is strong evidence to reject the null hypothesis of
    no linear relationship between *SATM* and *FYGPA* ($t_997=8.04$, p-value<0.0001)and conclude that, in
...
\indent The confidence intervals also help us pin down the uncertainty in each
...
```{r Figure8-16,fig.cap="(ref:fig8-16)",fig.height=5,fig.width=10}
...
mv1 <- as.tibble(predict(gpa1, newdata=dv1, interval="confidence"))
pv1 <- as.tibble(predict(gpa1, newdata=dv1, interval="prediction"))

mm1 <- as.tibble(predict(gpa1, newdata=dm1, interval="confidence"))
pm1 <- as.tibble(predict(gpa1, newdata=dm1, interval="prediction"))
...
\indent These results contrast with Figure \@ref(fig:Figure8-18) for the relationship
...
***indicator variables***^[Some people also call them ***dummy variables*** to reflect that they are stand-ins for dealing with the categorical information.]
...
```{r}
satGPA$SEX <- factor(satGPA$sex) #Make 1,2 coded sex into factor SEX
levels(satGPA$SEX) <- c("MALE", "FEMALE") #Make category names clear
SATSex1 <- lm(FYGPA~SATM+SEX, data=satGPA) #Fit lm with SATM and SEX
summary(SATSex1)
```

(ref:fig8-17) Plot of FYGPA vs SATV by Sex of students.

```{r Figure8-17,fig.cap="(ref:fig8-17)",echo=F}
scatterplot(FYGPA~SATV|sex, lwd=3, data=satGPA, spread=F,
            smooth=F, main="Scatterplot of GPA vs SATV by Sex",
            cex.main=1)
```
...
```{r Figure8-18,fig.cap="(ref:fig8-18)",echo=F}
scatterplot(FYGPA~SATM|sex, lwd=3, data=satGPA, spread=F,
            smooth=F, main="Scatterplot of GPA vs SATM by Sex",
            cex.main=1)
```
...
```{r}
satGPA$SEXINDICATOR <- as.numeric(satGPA$SEX=="FEMALE") #Convert logical to 0 for male, 1 for female
head(tibble(SEX=satGPA$SEX, SEXINDICATOR=satGPA$SEXINDICATOR), 10) #Explore first few observations
```
...
\indent To make this visually clearer, Figure \@ref(fig:Figure8-19) contains the
...
save them as an image or they may (for some reason) lose the shaded bands in some word processing programs.]. 
...
if the null is true, the p-value is $<0.0001$. We have very strong evidence that there is a difference in the true
y-intercept in a *SATM* model for first year college GPA between *males* and *females*. The confidence
...
```{r Figure8-23,fig.cap="(ref:fig8-23)",warning=F,message=F}
...
```{r Figure8-24,fig.cap="(ref:fig8-24)",fig.width=8,fig.height=5}
par(mfrow=c(2,2), oma=c(0,0,2,0))
plot(head1,
...
$x=0$) is right in the center of the plot and actually interesting^[Standardizing quantitative predictor variables is popular in social sciences, often where the response variable is also standardized. In those situations, they generate what are called "standardized slopes" (https://en.wikipedia.org/wiki/Standardized_coefficient) that estimate the change in SDs in the response for a 1 SD change in the explanatory variable.]. 

(ref:fig8-25) Scatterplot for reading score versus nonverbal IQ by dyslexia
group. 

```{r Figure8-25,fig.cap="(ref:fig8-25)",echo=F}
require(smdata)
data("dyslexic3")
scatterplot(score~ziq|dys, xlab="Standardized nonverbal IQ scores",
            ...
            cex.main=0.75, cex.leg=0.75)
```

```{r eval=F}
require(smdata)
data("dyslexic3")
dyslexic3 <- as.tibble(dyslexic3)
scatterplot(score~ziq|dys, xlab="Standardized nonverbal IQ scores",
            ylab="Reading score", data=dyslexic3, smooth=F,
            main="Plot of IQ vs Reading by dyslexia status")
```
...
```{r Figure8-26,fig.cap="(ref:fig8-26)",fig.width=8,fig.height=5}
par(mfrow=c(2,2), oma=c(0,0,2,0))
plot(dys_model,
...
```{r Figure8-27,fig.cap="(ref:fig8-27)",warning=F,message=F}
...
complicated of a model^[There is a way to test for a difference in the two lines at a particular $x$ value but it is beyond the scope of this material and uses what are called "constrasts".]. 
...
```{r Figure8-29,fig.cap="(ref:fig8-29)",warning=F,message=F}
...
\indent Since the IQ term has a large p-value, then we could drop it from the 
...
```{r Figure8-30,fig.cap="(ref:fig8-30)",warning=F,message=F}
...
encountered^[This is an example of what is called "step down" testing for model refinement which is a commonly used technique for arriving at a final model to describe response variables. Note that each step in the process should be reported, not just the final model that variables with small p-values remaining in it.]. 
...
```{r Figure8-31,fig.cap="(ref:fig8-31)",fig.height=4,fig.width=7,echo=F}
...
```{r Figure8-32,fig.cap="(ref:fig8-32)",echo=F}
...
```{r Figure8-33,fig.cap="(ref:fig8-33)",echo=F,warning=F,message=F}
...
\indent To perform the first test (after checking that assumptions are met, of
course),
...
result as: there is little to no evidence of a difference in the mean *du2* for the
...
\indent In the same ANOVA table, there is a test for the *du1* effect. This tests 
...
\indent There are some other facets of this discussion to keep in mind when comparing
...
support for having something in the model if there is better support for
other models than this simplest possible model. 

\indent In reading ``dredge`` output as it is constructed here, the models are sorted by
...
\indent There are two models that are clearly favored over the others with 
...
```{r Figure8-34,fig.cap="(ref:fig8-34)",echo=F,fig.height=4,fig.width=7}
require(coneproj)
data(FEV)
FEV<-as.tibble(FEV)
...
scatterplot(log(FEV)~age|smoke, data=FEV, smooth=F,
            ...
            legend.coords="topleft",cex.leg=0.75)
```
...
```{r Figure8-35,fig.cap="(ref:fig8-35)",fig.height=5,fig.width=8}
...
\indent This table also provides the opportunity to compare the model selection
results
...
```{r Figure8-36,fig.cap="(ref:fig8-36)",fig.height=4.5,fig.width=7}
fm1R$coefficients
confint(fm1R)
...
\indent The AICs are a preferred modeling strategy in some areas such as Ecology. As
...
\indent This chapter explored the most complicated models we're
...
dredge(`r colFmt("FULL\\_MODELNAME","red")`, rank="AIC", extra=c("R^2", 
