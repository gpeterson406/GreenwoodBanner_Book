...
\indent In this chapter, the focus shifts briefly from analyzing quantitative 
...
the value of quantitative variables^[Quantitative variables are displayed with boxplot-like bounds to describe the variability in the variable for that row of responses if for larger data sets.]. It also plots a red cell if the
...
```{r Figure5-1,fig.cap="(ref:fig5-1)", fig.height=5,fig.width=10}
require(vcd)
...
\noindent the ***column totals*** as the sum across the rows for the counts in column $c$ as

$$\mathbf{n_{\bullet c}}=\Sigma^R_{r=1}n_{rc},$$

\noindent and the ***table total*** as
...
\indent If we define some additional notation, we can then define hypotheses that allow us
...
\indent When we take a single random sample of size $N$ and make a contingency 
...
```{r}
require(poLCA)
data(election,package="poLCA") #2000 Survey - use package="" because other data sets in R have same name
...
\indent This section is very short in this chapter because we really do not use any 
...
\indent In order to assess the evidence against our null hypotheses of 
...
\noindent which is the sum over all ($R$ times $C$) cells in the contingency 
...
\noindent **square them**, and then we need to add them **all** up. In the 
...
\noindent which, when squared (in R, squaring is accomplished using ``^2``),
...
\indent When one additional assumption beyond the previous assumptions for 
the permutation
...
\indent Small p-values are generated by large $X^2$ values. If we want to understand
...
\noindent provides a measure of deviation of the observed from expected which 
...
\indent As introduced in Section \@ref(section5-3), a national random sample 
...
4. **Find the p-value:**
    ...
    * But since the expected cell count condition is violated, we should 
    ...
    ```{r Figure5-16,fig.cap="Permutation distribution of $X^2$ for the election data. Observed value of 763 not displayed."}
...
\indent A study of student behavior was performed at a university with a survey of 
...
4. **Find the p-value:**
    ...
    * But since the expected cell condition is violated, we 
    ...
    ```{r Figure5-22,fig.cap="Plot of permutation distributions for cheat/lie results with observed value of 13.24 (bold, vertical line)."}
...
\indent In recent decades, there has been a push for quantification of school performance
...
\item One way to get around the normality assumption is to use a method 
...
```{r Figure5-25,fig.cap="(ref:fig5-25)",echo=F,fig.width=6,fig.height=3.75}
old.par <- par(mai=c(0.5,0.5,0.5,0.5))
...
(-47.6, 6.75] for "growth" between -47.6 and 6.75) is least frequently occuring in *Elementary*
...
\indent Chi-square tests can be generally used to perform two types of tests,
...
require(tabplot)
csd$careabout<-factor(csd$careabout) #Need to make it explicit that these are factor variables
csd$careabout <- factor(csd$careabout,
                    levels=levels(csd$careabout)[c(1,4,3,2)]) #Reorders factor levels to be in "correct" order
...
tableplot(csd[,c("isare","careabout","thoughtabout","Gender",
                 "Age","Household.Income","Education")],sortCol=careabout) #Sorts plot by careabout responses
```
