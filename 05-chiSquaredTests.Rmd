...
```{r Figure5-5,fig.cap="(ref:fig5-5)",message=F,warning=F}
...
```{r Figure5-6,fig.cap="(ref:fig5-6)",echo=F,message=F,warning=F}
...
```{r Figure5-7,fig.cap="(ref:fig5-7)",echo=F,message=F,warning=F}
ArthritisFAKE <- rbind(Arthritis,Arthritis) # Just to make the following plot!
ArthritisFAKE$Treat<-factor(c(rep("Placebo",84),rep("Treated",84))) #Just to make the following plot
plot(Improved~Treat,data=ArthritisFAKE,main="Homogeneity Null Hypothesis True")
```
...
```{r Table5-3,echo=F}
columnHeaders <- c("None","Some","Marked","Totals")
rowLabels <- c("**Placebo**","**Treated**","**Totals**")
cell11 <- paste("$\\mathbf{\\frac{n_{\\text{Placebo}\\bullet}",
"*n_{\\bullet\\text{None}}}{N}\\\\",
"=\\frac{43*42}{84}}\\\\=\\color{red}{\\mathbf{21.5}}$",sep="")
cell12<- paste("$\\mathbf{\\frac{n_{\\text{Placebo}\\bullet}",
"*n_{\\bullet\\text{Some}}}{N}\\\\",
"=\\frac{43*14}{84}}\\\\=\\color{red}{\\mathbf{7.167}}$",sep="")
cell13<- paste("$\\mathbf{\\frac{n_{\\text{Placebo}\\bullet}",
"*n_{\\bullet\\text{Marked}}}{N}\\\\",
"=\\frac{43*28}{84}}\\\\=\\color{red}{\\mathbf{14.33}}$",sep="")
cell14 <- "$\\mathbf{n_{\\text{Placebo}\\bullet}=43}$"
row1 <- c(cell11,cell12,cell13,cell14)

cell21 <- paste("$\\mathbf{\\frac{n_{\\text{Treated}\\bullet}",
"*n_{\\bullet\\text{None}}}{N}\\\\",
"=\\frac{41*42}{84}}\\\\=\\color{red}{\\mathbf{20.5}}$",sep="")
cell22<- paste("$\\mathbf{\\frac{n_{\\text{Treated}\\bullet}",
"*n_{\\bullet\\text{Some}}}{N}\\\\",
"=\\frac{41*14}{84}}\\\\=\\color{red}{\\mathbf{6.83}}$",sep="")
cell23<- paste("$\\mathbf{\\frac{n_{\\text{Treated}\\bullet}",
"*n_{\\bullet\\text{Marked}}}{N}\\\\",
"=\\frac{41*28}{84}}\\\\=\\color{red}{\\mathbf{13.67}}$",sep="")
cell24 <- "$\\mathbf{n_{\\text{Treated}\\bullet}=41}$"
row2 <- c(cell21,cell22,cell23,cell24)

cell31 <- "$\\mathbf{n_{\\bullet\\text{None}}=42}$"
cell32 <- "$\\mathbf{n_{\\bullet\\text{Some}}=14}$"
cell33 <- "$\\mathbf{n_{\\bullet\\text{Marked}}=28}$"
cell34 <- "$\\mathbf{N=84}$"
row3 <- c(cell31,cell32,cell33,cell34)

t <- matrix(c(row1,row2,row3),ncol = 4,byrow=T,
            dimnames = list(rowLabels,columnHeaders))
kable(t,caption="(ref:tab5-3)")
```
...
row2 <- c(cell21,cell22,cell23)

t <- matrix(c(row1,row2),ncol = 3,byrow=T,
            dimnames = list(rowLabels,columnHeaders))
kable(t,caption="(ref:tab5-4)")
```
...
```{r Figure5-8,fig.cap="(ref:fig5-8)",warning=F,message=F}
set.seed(98765)
Arthperm$PermTreatment<-factor(shuffle(Arthperm$Treatment))
par(mfrow=c(2,2))
...
```{r Figure5-9,fig.cap="(ref:fig5-9)",message=F,warning=F,echo=F}
...
`r pval`). This suggests that our observed result is quite extreme 
...
```{r Figure5-11,fig.cap="(ref:fig5-11)",echo=F,message=F,warning=F}
plot(Improved~Treatment, data=Arthritis,
     main="Stacked Bar Chart of Arthritis Data")
```
...
```{r Figure5-12,fig.cap="(ref:fig5-12)",warning=F,message=F}
...
```{r Figure5-13,fig.cap="(ref:fig5-13)",message=F,warning=F}
require(poLCA)
data(election) #2000 Survey 
require(tabplot)
...
```{r Figure5-14,fig.cap="(ref:fig5-14)",warning=F,message=F}
...
4. **Find the p-value:**
    ...
    * The parametric p-value is <2.2e-16 from the R output which 
    ...
    ```{r Figure5-15,fig.cap="(ref:fig5-15)",echo=F,warning=F,message=F}
    ...
    * But since the expected cell count condition is violated, we should 
    ...
    ```{r Figure5-16,fig.cap="(ref:fig5-16)",message=F,warning=F}
...
```{r Figure5-17,fig.cap="(ref:fig5-17)",warning=F,message=F}
...
```{r Figure5-18,fig.cap="(ref:fig5-18)",warning=FALSE,message=F}
...
```{r Figure5-19,fig.cap="(ref:fig5-19)",message=F,warning=F}
...
```{r Figure5-20,fig.cap="(ref:fig5-20)",warning=F,message=F}
    ...
    * All expected cell counts larger than 5 (required to use
    $\chi^2$-distribution to find p-values):
        ...
        * There are three expected cell counts (in bold) below 5, 
        ...
        require(pander)
        x <- chisq.test(cheatlietable)$expected
        emphasize.strong.cells(which(x <=5, arr.ind=T))
        pander(x)
        ```
...
4. **Find the p-value:**
    ...
    * The parametric p-value is 0.0102 from the R output. This was based
    ...
    ```{r Figure5-21,fig.cap="(ref:fig5-21)",echo=F,warning=F,message=F}
    ...
    * But since the expected cell condition is violated again, we 
    ...
    ```{r Figure5-22,fig.cap="(ref:fig5-22)", warning=F,message=F}
...
5. **Make a decision:**
    ...
    * With a p-value of `r pval`, we can say that there is a 
    `r pval*100`% chance of observing a configuration like ours or more
...
```{r echo=F,warning=F,message=F}
require(pander)
x <- chisq.test(cheatlietable)$residuals
y <- round(x,4)
emphasize.strong.cells(which(abs(y)>=2,arr.ind = TRUE))
pander(y)
```
...
```{r Figure5-23,fig.cap="(ref:fig5-23)",message=F,warning=F}
require(survey)
data(api)
...
```{r Figure5-24,fig.cap="(ref:fig5-24)",warning=F,message=F}
...
```{r Figure5-25,fig.cap="(ref:fig5-25)",message=F,warning=F}
...
```{r Figure5-26,fig.cap="(ref:fig5-26)"}
...
`r colFmt("y","red")`, data=`r colFmt("DATASETNAME","red")`, margins=F)
