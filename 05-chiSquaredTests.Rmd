...
colnames(m) <- columnHeaders <- c("","**Response\\\n Level 1**", 
                   ...
                   "**Response\\\n Level C**",
                   "\\\n **Totals**")
pandoc.table(m, keep.line.breaks = TRUE,caption="(\\#tab:Table5-2) My table caption", split.table = Inf)
```
...
kable(x,caption="(ref:tab5-2)",escape=F)
```
...
t <- matrix(c(row1,row2,row3),ncol = 4,byrow=T,
            dimnames = list(rowLabels,columnHeaders))
kable(t,caption="(ref:tab5-3)",escape=F)
```
...
t <- matrix(c(row1,row2),ncol = 3,byrow=T,
            dimnames = list(rowLabels,columnHeaders))
kable(t,caption="(ref:tab5-4)",escape=F)
```
