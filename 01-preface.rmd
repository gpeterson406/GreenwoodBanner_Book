...
Figure \@ref(fig:Figure1-1) frames the topics we will discuss. Taking a broad
...
```{r Figure1-1, echo=FALSE, fig.cap ="(ref:fig1-1)"}
knitr::include_graphics('chapter1_files/image002.png')
```
...
```{r Figure1-2, echo=FALSE, fig.cap="(ref:fig1-2)"}
knitr::include_graphics('chapter1_files/fig1.2.png')
```
...
The following code to read in the data set relies on an R package called ``readr`` [@R-readr]. Packages in R provide additional functions and data sets that are not available in the initial download of R or RStudio. To get access to the packages, first "install" (basically
...
```{r include=F}
treadmill <- read_csv("chapter1_files/treadmill.csv")
```
...
```{r Figure1-3, echo=FALSE, fig.cap="(ref:fig1-3)"}
knitr::include_graphics('chapter1_files/fig1.3.png')
```
...
Figure \@ref(fig:Figure1-4) shows the screen with the code used in this 
...
```{r Figure1-4, echo=FALSE, fig.cap="(ref:fig1-4)"}
knitr::include_graphics('chapter1_files/fig1.4.png')
```
...
[1] 1.387414
```
\noindent And now we know that the average running time for 1.5 miles for the subjects in the study was 10.6 minutes with a standard deviation (SD) of 1.39 minutes. But you should remember that the
...
```{r Figure1-5, fig.cap='(ref:fig1-5)',echo=F}
...
parts of this process in the screen grab in Figure \@ref(fig:Figure1-6). 

(ref:fig1-6) RStudio while in the process of copying the histogram.

```{r Figure1-6,echo=FALSE,fig.cap="(ref:fig1-6)",fig.width=6,fig.height=3.5}
knitr::include_graphics('chapter1_files/image010.png')
```

\noindent You can also directly save the figures as separate files using 
...
```{r Figure1-7, fig.cap="(ref:fig1-7)",echo=F,fig.width=4,fig.height=2.75}
hist(treadmill$RunTime, labels=T)
```
...
```{r Figure1-8, fig.cap="(ref:fig1-8)",echo=F,fig.width=4,fig.height=2.5}
old.par <- par(mai=c(0.5,0.5,0.5,0.5))
...
```{r Figure1-9, fig.cap="(ref:fig1-9)",echo=F,fig.width=4,fig.height=2.5}
...
All the previous results were created by running the R code and then copying the
...
* `r colFmt("FILENAME","red")` ``<-`` **read.csv(**`r colFmt('"path to csv file/FILENAME.csv"','red')`**)**
