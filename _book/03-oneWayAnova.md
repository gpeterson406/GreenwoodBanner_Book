...
***Analysis of Variance***, or ***ANOVA***, that directly assesses whether there is
...
coefficeints, so only those lines are output. When fitting this version
...
column ("Estimate" column) of the coefficient table. 
...
Now consider $\text{SS}_E = \Sigma^J_{j=1}\Sigma^{n_j}_{i=1}(y_{ij}-\bar{y}_j)^2$
...
the residuals $(e_{ij} = y_{ij}-\hat{y}_{ij})$ on the y-axis and the fitted values
...
would generate. For example, the point labeled as "82" (the 82^nd^
...
4. **Find the p-value**:
    ...
    * The parametric approach is easiest -- the p-value comes from the previous 
    ...
    that the calculation is at the numerical precision     of the computer. 
    ...
    **When you encounter p-values that are smaller than 0.0001, you should just
    ...
    distribution (the distribution of the test statistic if the null hypothesis
    is true). 
    ...
    * **The permutation p-value was reported as 0. This should be reported as 
    p-value < 0.001** since we did 1000 permutations and found that none of the
    ...
    * There is evidence at the 5% significance level that the different treatments 
    ...
    mean tooth growth for **these** guinea pigs. 
        ...
        * The alternative is that there is some difference in the true means 
        ...
        $F$-test is about finding evidence of some means. The next section will
        provide some additional tools to get more specific about the source of 
        those detected differences. 
...
false detection grows rapidly with the number of tests. The plot stops at 100
...
next data set they collect.^[Many researchers are now collecting multiple data 
...
initial analysis to verify that the first results are also present in that 
second data set.] 
...
other and VC.1 and OJ.0.5 are also not detectably different. If you look carefully, 
