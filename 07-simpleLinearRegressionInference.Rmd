...
\noindent which can be read as "y follows a normal distribution
...
\noindent so we need to obtain the appropriate standard error for regression model
...
\noindent is a test for **no linear relationship between** $\mathbf{x}$ **and** $\mathbf{y}$
...
\noindent with the main interest in the test for $\beta_1$ based on $b_1$ for now.
...
```{r Figure7-8,fig.cap="(ref:fig7-8)",warning=F,message=F}
...
```{r Figure7-9,fig.cap="(ref:fig7-9)",warning=F,message=F}
Tobs <- lm(meanmax~Year, data=bozemantemps)$coef[2]
Tobs
par(mfrow=c(1,2))
B <- 1000
...
```{r Figure7-10,fig.cap="(ref:fig7-10)",echo=F,fig.width=5,fig.height=4}
...
plot(meanmax~Year,data=bozemantemps,xlab="Year",ylab="degrees F",
     main="degrees F and Year",cex.main=0.75)
...
```{r Figure7-11,fig.cap="(ref:fig7-11)",fig.height=4,fig.width=5}
par(mfrow=c(1,2))
...
```{r Figure7-17,fig.cap="(ref:fig7-17)",echo=F,message=F,warning=F}
par(mfrow=c(1,2))
...
```{r Figure7-18,fig.cap="(ref:fig7-18)",echo=F,warning=F,message=F}
data(UN)
...
\noindent where $\hat{\sigma}^2$ is the squared residual standard error. This formula
...
```{r Figure7-22,fig.cap="(ref:fig7-22)",warning=F,message=F}
par(mfrow=c(1,1))
...
```{r Figure7-23,fig.cap="(ref:fig7-23)",echo=F,warning=F,message=F}
temp1<-lm(meanmax~Year, data=bozemantemps)
...
* `r colFmt("DATASETNAME\\$logy","red")` <- log(`r colFmt("DATASETNAME\\$y","red")`)
...
newdata=data.frame(`r colFmt("x","red")` = `r colFmt("XNEW","red")`),
interval="confidence")
