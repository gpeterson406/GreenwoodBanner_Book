...
require(mosaic)
library(knitr)
knitr::opts_chunk$set(cache = T,fig.height=3.5,fig.width=6,fig.pos='HT')
...
* **Independence of observations**
    ...
    ***autocorrelation*** (correlation between neighboring time points or
    ...
    ```{r Figure7-5,fig.cap="(ref:fig7-5)",warning=F,message=F}
...
```{r Figure7-7,fig.cap="(ref:fig7-7)",echo=F,warning=F,message=F}
set.seed(1667)
...
```{r Figure7-10,fig.cap="(ref:fig7-10)",warning=F,message=F,echo=F}
...
kable(x,caption="(ref:tab7-1)",col.names=columnHeaders)
```
...
```{r Figure7-11,fig.cap="(ref:fig7-11)",message=F,warning=F}
par(mfrow=c(1,2))
...
```{r Figure7-12,fig.cap="(ref:fig7-12)",echo=F,warning=F,message=F}
require(spuRs) # install.packages("spuRs")
data(ufc)
require(car)
scatterplot(height.m~dbh.cm, data=ufc[-168,], smooth=T, spread=F,
...
```{r Figure7-13,fig.cap="(ref:fig7-13)",echo=F,warning=F,message=F}
scatterplot(log(height.m)~dbh.cm, data=ufc[-168,], smooth=T, spread=F,
            main="log-Tree height vs tree diameter")
```
...
```{r Figure7-14,fig.cap="(ref:fig7-14)",echo=F,warning=F,message=F}
...
```{r Figure7-16,fig.cap="(ref:fig7-16)",echo=F,warning=F,message=F}
require(car)
data(UN)
par(mfrow=c(2,2))
...
```{r Figure7-19,fig.cap="(ref:fig7-19)",message=F,warning=F}
par(mfrow=c(2,2))
plot(ID1)
```
...
```{r Figure7-20,fig.cap='(ref:fig7-20)',warning=F,message=F,echo=F}
par(mfrow=c(1,2))

plot(log(infant.mortality)~log(gdp),data=UN,pch=16,col="coral",xlab="log-GDP",ylab="log-Infant deaths/1000 births",main="(a) log-Infant Mort. vs log-GDP")
...
```{r Figure7-21,fig.cap="(ref:fig7-21)",echo=F,message=F,warning=F}
par(mfrow=c(2,2))
plot(ID2)
```
