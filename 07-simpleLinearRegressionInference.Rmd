...
the explanatory variable^[We can also write this as $E(y_i|x_i) = \mu\{y_i|x_i\} = \beta_0 + \beta_1x_i, which is the notation you will see if you continue into the *Methods of Data Analysis I* course. We will use notation that is consistent with how we originally introduced the methods.]. 
...
shape and variance that is an assumption of the regression model. \indent Figure \@ref(fig:Figure7-2) contains simulated realizations from a normal
...
(ref:fig7-3b) Graphic illustrating the confidence interval for a slope coefficient for a 1 unit increase in $x$.

```{r Figure7-3b,echo=F,message=F,warning=F,fig.align="center"}
...
\indent Test statistics for regression coefficients are developed, if assumptions 
...
0.5 and it would be easiest to look it up directly using ``pt`` in the direction of the alternative.
...
\indent The width of the CI, loosely the precision of the estimated slope, is
...
would just report
"p-value < 0.0001". This suggests that there is very strong evidence of a linear
...
to perform a hypothesis test. Either way, we would conclude that there is little evidence to conclude that there is some linear
relationship between body fat and Hematocrit in the population of female
Australian athletes. If your standards were different, say if you think p-values around 0.10 provide moderate evidence, you might have a different opinion about
...
Decision: The p-value is 
very small so presents very strong evidence against $H_0$. 
...
Bozeman from 1900 to 2014 with SLR (thin, green) and smoothing (thick, red) lines.

```{r Figure7-4,fig.cap="(ref:fig7-4)",error=F,message=F,fig.height=5,fig.width=8}
...
*year* with some difference between the smoothing and SLR lines. If the curving is real, it
would suggest a slightly curving response over time -- the curve is relatively minor relative to the variability in the data set. There also appears to be one potential large outlier
in the late 1930s. 

\indent We'll perform all 6+ steps of the hypothesis test for the slope coefficient 
and add a confidence
interval interpretation for this example. First, we need to select our hypotheses (the 2-sided test
...
```{r Figure7-5,fig.cap="Diagnostic plots of the Bozeman yearly temperature simple linear  regression model.",fig.height=5,fig.width=8}
...
* **Linearity of relationship**
    ...
    * Examine the Residuals vs Fitted plot:
        ...
        * There does not appear to be a clear curve remaining in 
        ...
        and we should be able to proceed without worrying too much 
...
* **Normality of residuals**
    ...
    * Examining the Normal QQ-plot for violations of the normality 
    ...
    32^nd^ observation in the data set (1934)
...
5. **Make a decision:**
    ...
    The p-value is very small so provides very strong evidence against the null hypothesis of no trend over time.. 
...
tried to analyze the monthly or original daily maximum temperature data even though we might find about the same estimated change over time. 
...
Here, the 95% CI is (0.043, 0.062). This tells us that for a 1 year increase in *Year*, we are 95% confident that the change in the true mean of the yearly average maximum *Temperatures* in Bozeman is between 0.043 and 0.062 degrees F. 
...
\indent One issue that arises in local studies of climate change is that researchers 
...
multiple testing is using what is called a Bonferroni adjustment, 
...
``plot(allEffects(MODELNAME))`` although we enhanced our version a little. In 
...
```{r Figure7-6,fig.cap="(ref:fig7-6)",message=F,warning=F}
require(effects)
...
predictor variable's values is as good as any other. Consider the four scatterplots of
...
```{r Figure7-7,fig.cap="(ref:fig7-7)",echo=F,fig.height=5,fig.width=8}
set.seed(1667)
...
```{r Figure7-8,fig.cap="(ref:fig7-8)",fig.height=5,fig.width=10}
...
approach is not unreasonable except possibly for the independence assumption. These randomization approaches provide no robustness against violations of the independence assumption.
...
quantiles
hist(Tstar, labels=T, ylim=c(0,450))
...
(the focus of subsequent statistics courses). 

\indent `r textItalicBold("Transformations")` involve applying a function to 
one or both variables. After applying this transformation, one hopes to have
...
```{r Figure7-10,fig.cap="(ref:fig7-10)",echo=F,fig.width=10,fig.height=5}
...
log-transformation is so commonly used (and misused), that we will just focus
on its use. It is so commonplace in some fields that some researchers
log-transform their data prior to even plotting their data. In other
...
```{r Figure7-11,fig.cap="(ref:fig7-11)",fig.height=5,fig.width=10}
par(mfrow=c(1,2))
...
Figure \@ref(fig:Figure7-11)(a) displays a relationship
...
scatterplot(height.m~log(dbh.cm), data=ufc[-168,], smooth=T, spread=F,
...
```{r Figure7-15,fig.cap="(ref:fig7-15)",warning=F,message=F}
...
```{r Figure7-16,fig.cap="(ref:fig7-16)",echo=F,fig.height=5,fig.width=8}
require(car)
data(UN)
UN<-as.tibble(UN)
par(mfrow=c(2,2))
...
$\widehat{\text{infantmortality}}=168.648-16.6\cdot\log(\text{GDP})$. The first
...
```{r Figure7-19,fig.cap="(ref:fig7-19)",fig.height=5,fig.width=8}
par(mfrow=c(2,2))
plot(ID1)
```
...
```{r Figure7-20,fig.cap='(ref:fig7-20)',echo=F}
par(mfrow=c(1,2))
ID2 <- lm(log(infant.mortality)~log(gdp), data=UN)
plot(log(infant.mortality)~log(gdp),data=UN,pch=16,col="coral",xlab="log-GDP",ylab="log-Infant deaths/1000 births",main="(a) log-Infant Mort. vs log-GDP",cex.main=1)
...
actually made the resulting relationship look like it might be close to meeting the SLR
...
```{r Figure7-21,fig.cap="(ref:fig7-21)",echo=F,fig.height=5,fig.width=8}
ID2 <- lm(log(infant.mortality)~log(gdp), data=UN)
summary(ID2)
par(mfrow=c(2,2))
plot(ID2)
...
nomenclature was inspired by @DeVeaux2001 *Stats: Data and
...
intervals, we assume that a 95% PI "succeeds" -- now it contains the new observation --
...
```{r}
BBCI <- as.tibble(predict(m1, newdata=tibble(Beers=beerf),
                           interval="confidence"))
head(BBCI)
```
...
```{r}
BBPI <- as.tibble(predict(m1, newdata=tibble(Beers=beerf),
                           interval="prediction"))
head(BBPI)
```
...
```{r Figure7-22,fig.cap="(ref:fig7-22)",fig.height=5,fig.width=10,echo=F}
par(mfrow=c(1,1))
...
```{r Figure7-23,fig.cap="(ref:fig7-23)",echo=F,fig.height=5,fig.width=10}
temp1<-lm(meanmax~Year, data=bozemantemps)
 Yearf<-seq(from=1901,to=2014,length.out=75)
 
TCI<-as.tibble(predict(temp1,newdata= tibble(Year=Yearf),interval="confidence"))
#head(TCI)
TPI<-as.tibble(predict(temp1,newdata=tibble(Year=Yearf),interval="prediction"))
#head(TPI)
...
regression equations -- one for females and one for males. 
