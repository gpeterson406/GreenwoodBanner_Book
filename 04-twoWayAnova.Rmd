...
require(mosaic)
library(knitr)
knitr::opts_chunk$set(cache = TRUE,fig.height=3.5,fig.width=6,fig.pos='HT')
...
kable(x,caption="(ref:tab4-1)",col.names=columnHeaders,escape=F)
```
...
library(knitr)
kable(x,caption="(ref:tab4-2)",col.names=columnHeaders,escape=F)
```
...
pander(anova(m1),add.significance.stars=F,
      caption="Interaction ANOVA table with interaction row in bold.")
```
...
kable(t,caption="(ref:tab4-3)",escape=F,col.names = columnHeaders)
```
