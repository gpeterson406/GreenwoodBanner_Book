...
library(knitr)
library(readr)
knitr::opts_chunk$set(cache = T,fig.height=3.5,fig.width=6,fig.pos='HT',warning=F,
                      message=F)
...
\noindent with the regression notation from the MLR model with an interaction: 
...
```{r Figure9-3,fig.cap="(ref:fig9-3)",echo=F}
source("http://www.math.montana.edu/courses/s217/documents/intplot.R")
intplot(Massperha~Species*Treatment, data=gdn, col=c(1,2), lwd=2,cex.main=1)
```
...
```{r Figure9-4,fig.cap="(ref:fig9-4)",fig.height=5,fig.width=8}
...
```{r Figure9-5,fig.cap="(ref:fig9-5)"}
...
```{r Figure9-6,fig.cap="(ref:fig9-6)",fig.height=5,fig.width=8}
...
```{r Figure9-8,fig.cap="(ref:fig9-8)"}
...
```{r Figure9-10,fig.cap="(ref:fig9-10)",echo=F}
...
require(car)
scatterplot(logSpecies~logDBCs|TJK, data=bm, smooth=F, cex.main=0.75,cex.leg=0.5,
...
\noindent which was compared to 

$$\log{(\text{count})}_i=\beta_0 + \beta_1\log{(\text{DBF})}_i 
+ \beta_2I_{\text{TJK},i} + \varepsilon_i$$

\noindent as well as the simpler models that each suggests:
...
columnHeaders <- c("**Model**&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",
                   ...
                   "**df**","**logLik**&nbsp;","**AIC**",
                   "$\\BD$**AIC**")
cell11 <- "$\\log(\\text{count})_i=\\beta_0 + \\beta_1\\log(\\text{DBC})_i + \\beta_2I_{\\text{TJK},i} + \\varepsilon_i$"
cell21 <- "$\\log(\\text{count})_i=\\beta_0 + \\beta_1\\log(\\text{DBF})_i + \\beta_2I_{\\text{TJK},i} + \\varepsilon_i$"
cell31 <- "$\\log(\\text{count})_i=\\beta_0 + \\beta_1\\log(\\text{DBC})_i + \\varepsilon_i$"
cell41 <- "$\\log(\\text{count})_i=\\beta_0 + \\beta_1\\log(\\text{DBF})_i + \\varepsilon_i$"
...
```{r Figure9-11,fig.cap="(ref:fig9-11)",fig.height=5,fig.width=8}
par(mfrow=c(2,2), oma=c(0,0,2,0))
plot(bd1)
```
...
\noindent exploring whether *DBC* or *DBF* is the "better" with *TJK* in the model. There
