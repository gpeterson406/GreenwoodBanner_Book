...
data(MockJury)
par(mfrow=c(1,2))
#old.par <- par(mar=c(0.75,1,0.5,0.5))
...
\noindent or, in words, **at least one of the true means differs among the J groups**.
...
\indent The previous discussion showed two ways of parameterizing models for the 
...
$\text{SS}_A$ value. In the ANOVA table, it is the second number in the first row;  we can use the bracket, ``[,]``, referencing to extract that 
...
components. Note the general format of the ANOVA table is in Table \@ref(tab:tab3-2)^[Make sure you can work 
...
the selection^[This fits with a critique of p-value usage called p-hacking -- where researchers search across many results and only report their biggest differences.]. If the entire suite of pairwise comparisons are considered, this 
...
contribute much to the overall $F$-test. In Section 3.5, we will revisit this topic and consider a method that is
...
```{r Figure3-9,fig.cap="(ref:fig3-9)",echo=F,fig.width=8,fig.height=5}
par(mfrow=c(2,2))
plot(lm2, pch=16)
```
...
```{r Figure3-14, fig.cap="(ref:fig3-14)",fig.height=5,fig.width=10}
par(mfrow=c(1,2))
...
2. **Validity conditions**:
    ...
    * Constant variance:
        ...
        * As noted above, there is some indication of a difference in the 
        ...
        ```{r Figure3-15,fig.cap="Diagnostic plots for the toothgrowth model.",fig.height=5,fig.width=8}
...
4. **Find the p-value**:
    ...
    * The nonparametric approach is not too hard so we can compare the two approaches here 
    ...
    ```{r Figure3-16, fig.cap="Histogram and density curve of permutation distribution for $F$-statistic for tooth growth data. Observed test statistic in bold, vertical line at 41.56."}
...
\noindent where $\frac{q^*}{\sqrt{2}}\sqrt{\text{MS}_E\left(\frac{1}{n_j}+
