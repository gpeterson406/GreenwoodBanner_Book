...
par(mfrow=c(2,2))
require(gplots)
plotmeans(Years~Attr,data=MockJury,ylab="Years",mean.labels=T,digits=2,legends=c("Beautiful","Average","Unattractive"),barcol="red",col="black", main="(a) Real Data",ylim=c(3,7.2))
anova(lm(Years~Attr,data=MockJury))
...
```{r warning=F, message=F, echo=T, fig.width=12, fig.height=8}
...
tally(~Treat,data=ToothGrowth) #New variable that combines supplement type and dosage

par(mfrow=c(1,2))
boxplot(len~Treat,data=ToothGrowth,ylab="Tooth Growth in microns")
beanplot(len~Treat,data=ToothGrowth,log="",col="yellow",method="jitter",ylab="Tooth Growth in microns")
