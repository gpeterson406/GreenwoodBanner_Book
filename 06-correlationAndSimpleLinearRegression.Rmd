...
\indent The independence test in Chapter \@ref(chapter5) provided a technique for assessing evidence of a
...
\indent In terms of quantifying relationships between variables, we start with 
...
\indent In assessing the relationship
...
the linear relationships $\boldsymbol{r}=0.20$ for women and
$\boldsymbol{r}=0.45$ for men). Especially for male athletes, those that are 
...
\indent We used bootstrapping briefly in
...
$\boldsymbol{r}=0.89$. As correlations approach -1 or 1, the sampling becomes 
...
\indent The 95% confidence level corresponds to a 5% significance level test. The 95% CI is
...
\indent In a study at the Upper Flat Creek
...
\indent When the relationship appears to
...
```{r Figure6-15,fig.cap="(ref:fig6-15)",warning=F,message=F}
aisR2 <- ais[-c(56,166), c("Ht","Hc","Bfat","Sex")]
...
\indent The previous results used the ``lm`` function as a "black box" to generate
...
\indent At the beginning of the chapter, 
...
**R^2^** is calculated using the sums of squares we encountered in the
...
that is in the second to last line of the output below. It is reported as a 
...
\indent In the review of correlation, we
...
\indent Influential points are not the
...
* **Beers vs BAC:**
    ...
    * Independence of observations:
        ...
        * We can assume that all the subjects are independent of each 
        ...
        ```{r Figure6-22,fig.cap="Full suite of diagnostics plots for *Beer* vs *BAC* data.",fig.height=5,fig.width=8}
        m1 <- lm(BAC~Beers, data=BB)
        par(mfrow=c(2,2))
...
```{r Figure6-23,fig.cap="(ref:fig6-23)",fig.height=5,fig.width=8}
par(mfrow=c(2,2))
...
\indent A study in August 1985 considered time for Old Faithful and how that might 
...
G2 <- tibble(Waiting=geyser$waiting[-1],
...
```{r Figure6-25,fig.cap="(ref:fig6-25)",fig.height=5,fig.width=8}
par(mfrow=c(2,2))
plot(OF1)
```
...
\indent The correlation coefficient ($\boldsymbol{r}$ or Pearson's Product Moment 
