...
row6 <- c("$\\ldots$","$\\ldots$","$\\ldots$","$\\ldots$","$\\ldots$",
          "$\\ldots$","$\\ldots$")
...
```{r Figure2-6, fig.cap="(ref:fig2-6)"}
par(mfrow=c(1,2))
...
```{r Figure2-7, echo=FALSE, fig.cap ="(ref:fig2-7)",fig.height=4}
...
```{r Figure2-9, fig.cap="(ref:fig2-9)"}
par(mfrow=c(1,2))
B <- 1000
Tstar <- matrix(NA, nrow=B)
for (b in (1:B)){
...
```{r Figure2-10, fig.cap="(ref:fig2-10)", fig.width=3, fig.height=3.5}
par(mfrow=c(1,2))
Tobs <- 1.837
hist(Tstar, labels=T)
...
```{r Figure2-11, fig.cap="(ref:fig2-11)"}
par(mfrow=c(1,2))
hist(Tstar, labels=T)
...
```{r Table2-2, echo=F}
columnHeaders <- c("Random Sampling/Random Assignment",
...
cell32 <- 'Cannot generalize inference to the population of interest because the sample was not random and could be biased -- may not be "representative" of the population of interest. Can establish causal inference due to RA $\\rightarrow$ the inference from this type of study applies only to the sample.'
...
x <- matrix(c(cell21,cell22,cell23,cell31,cell32,cell33),byrow=T,ncol=3)
kable(x,caption='(ref:tab2-2)',escape=F,col.names = columnHeaders)
```
...
```{r Table2-3, echo=F}
columnHeaders <- c("$\\mathbf{H_0}\\textbf{ True}$","$\\mathbf{H_0}\\textbf{ False}$")
...
library(knitr)
kable(x,caption="(ref:tab2-3)",escape=F)
```
...
```{r Figure2-12, fig.cap="(ref:fig2-12)", fig.width=3}
par(mfrow=c(1,2))
...
```{r Figure2-20,fig.cap="(ref:fig2-20)",fig.height=2.5}
par(mfrow=c(1,1))
