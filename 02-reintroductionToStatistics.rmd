...
When you run these three lines ofcode, the console will show you the following 
output:
...
``shuffle`` function performs permutations ofthe group labels relative to 
...
finding the difference in themeans, storing it in a variable called ``Ts`` is:
...
Now we can run our loop B times andstore the results in ``Tstar``. 
...
as failing to convict a truley guilty person. This most often occurs due to a 
...
```{r Table2-2, echo=F}
columnHeaders <- c("","**H<sub>0</sub> True**","**H<sub>0</sub> False**")
cell21 <- '**FTR H<sub>0</sub>**'
cell22 <- 'Correct decision'
cell23 <- 'Type II error'
cell31 <- '**Reject H<sub>0</sub>** '
cell32 <- 'Type I error'
cell33 <- 'Correct decision'
x <- matrix(c(cell21,cell22,cell23,cell31,cell32,cell33),nrow=,ncol=3,byrow=TRUE)
library(knitr)
kable(x,col.names=columnHeaders,caption='(ref:tab2-2)')
```
...
We get a similar^[We rounded the means a little and that causd the small
...
* ``summary``(<span style='color:red'>``DATASETNAME``</span>)
