...
```{r Figure1, echo=FALSE, fig.cap ="(ref:fig1)"}
knitr::include_graphics('chapter0_files/image001.png')
```
...
```{r Figure2, echo=FALSE, fig.cap="(ref:fig2)"}
knitr::include_graphics('chapter0_files/image003.png')
```
...
```{r include=F}
treadmill <- read.csv("chapter0_files/treadmill.csv")
```
...
```{r Figure3, echo=FALSE, fig.cap="(ref:fig3)"}
knitr::include_graphics('chapter0_files/image005.png')
```
...
```{r Figure4, echo=FALSE, fig.cap="(ref:fig4)"}
knitr::include_graphics('chapter0_files/image006.png')
```
...
```{r Figure6,echo=FALSE,fig.cap="(ref:fig6)"}
knitr::include_graphics('chapter0_files/image010.png')
```
...
```{r Figure2-9, fig.cap="(ref:fig2-9)", fig.show="hold", fig.width=3}
B <- 1000
Tstar <- matrix(NA, nrow=B)
for (b in (1:B)){
  Tstar[b] <- diffmean(Years ~ shuffle(Attr), data=MockJury2)
...
`r pdata(Tstar,1.837,lower.tail=F)*1000` $T^*$ values over 2 based on the 
...
```{r Figure2-10, fig.cap="(ref:fig2-10)", fig.show="hold", fig.width=3}
Tobs <- 1.837
hist(Tstar, labels=T)
...
remember, are the probability of getting a result as extreme as or more extreme than what we observed, <u>given that the null is true</u>. Finding only `r pval*1000`
...
```{r Table2-1, echo=F}
columnHeaders <- c("**Random Sampling/Random Assignment**","**Random Assignment (RA) -- Yes (controlled experiment)**","**Random Assignment (RA) -- No (observational study)**")
cell21 <- '**Random Sampling (RS) -- Yes (or some method that results in a representative sample of population of interest)**'
cell22 <- 'Because we have RS, we can generalize inferences to the population the RS was taken from. Because we have RA we can assume the groups were equivalent on all aspects except for the treatment and can establish causal inference. '
cell23 <- 'Can generalize inference to population the RS was taken from but cannot establish causal inference (no RA - cannot isolate treatment variable as only difference among groups, could be confounding variables).'
cell31 <- '**Random Sampling (RS) -- No (usually a convenience sample)**'
cell32 <- 'Cannot generalize inference to the population of interest because the sample was not random and could be biased - may not be "representative" of the population of interest. Can establish causal inference due to RA 	&rarr; the inference from this type of study applies only to the sample.'
cell33 <- 'Cannot generalize inference to the population of interest because the sample was not random and could be biased - may not be "representative" of the population of interest. Cannot establish causal inference due to lack of RA of the treatment.'
x <- matrix(c(cell21,cell22,cell23,cell31,cell32,cell33),nrow=,ncol=3,byrow=TRUE)
library(knitr)
kable(x,col.names=columnHeaders,caption='(ref:tab2-1)')
```
...
cell21 <- "$\\textbf{FTR }\\mathbf{H_0}$"
cell22 <- 'Correct decision'
cell23 <- 'Type II error'
cell31 <- "$\\textbf{Reject }\\mathbf{H_0}$"
cell32 <- 'Type I error'
cell33 <- 'Correct decision'
x <- matrix(c(cell22,cell23,cell32,cell33),
            dimnames = list(c(cell21,cell31),columnHeaders),ncol=2,byrow=TRUE)
library(knitr)
kable(x,caption="(ref:tab2-2)")
```
...
```{r Figure2-12, fig.cap="(ref:fig2-12)", fig.show="hold", fig.width=3}
...
```{r Figure2-13, echo=FALSE, fig.cap ="(ref:fig2-13)"}
knitr::include_graphics('chapter1_files/image045.png')
```
...
```{r Figure2-14, echo=FALSE, fig.cap ="(ref:fig2-14)"}
knitr::include_graphics('chapter1_files/image047.png')
```
...
```{r Figure2-15, fig.cap="(ref:fig2-15)", warning=F,message=F}
...
```{r Figure2-16, fig.cap="(ref:fig2-16)"}
par(mfrow=c(1,2))
hist(Tstar,labels=T)
abline(v=c(-1,1)*Tobs, lwd=2, col="red")
plot(density(Tstar), main="Density curve of Tstar")
...
```{r Figure2-17, fig.cap="(ref:fig2-17)",echo=F}
...
```{r Figure2-18, fig.cap="(ref:fig2-18)",results='hold'}
par(mfrow=c(1,2))
...
```{r Figure2-19, fig.cap="(ref:fig2-19)"}
par(mfrow=c(1,2))
hist(Tstar, labels=T)
...
```{r Figure2-20,fig.cap="(ref:fig2-20)"}
par(mfrow=c(1,1))
...
```{r echo=F}
t <- favstats(Years~Attr, data=MockJury2)
pander(t, digits=3,emphasize.strong.cols=c(1,7,8,9))
```
...
```{r Figure2-21,fig.cap="(ref:fig2-21)"}
par(mfrow=c(1,2))
hist(Tstar,labels=T)
abline(v=Tobs,col="red",lwd=2)
...
* beanplot(`r colFmt("Y","red")`~`r colFmt("X","red")`, data=`r colFmt("DATASETNAME","red")`)
...
```{r Figure4-10,fig.cap="(ref:fig4-10)",warning=F,message=F}
...
```{r Figure4-11,fig.cap="(ref:fig4-11)",message=F,warning=F}
...
```{r Figure4-12,fig.cap="(ref:fig4-12)",message=F,warning=F}
require(faraway)
data(debt)
...
```{r Figure4-13,fig.cap="(ref:fig4-13)"}
par(mfrow=c(2,2))
plot(debt1)
```
...
2. **Validity conditions -- discussed above but with new plots for the additive:**
    ...
    ```{r Figure4-14,fig.cap="(ref:fig4-14)"}
...
panel for income gropu and the right panel for buying cigarettes or not
(1 for yes).

```{r Figure4-15,fig.cap="(ref:fig4-15)"}
...
six combinations can be found in Table \@ref(tab:Table4-2).

(ref:tab4-2) Calculations to construct the estimates for all combinations 
...
addtorow$pos[[2]] <- c(-1) #multicol
addtorow$pos[[3]] <- c(0)  #midrule
addtorow$pos[[5]] <- c(nrow(out.x)) #endrule

addtorow$command <- c('\\toprule\n',' & & & \\multicolumn{5}{c}{Income Group} (\\%)} & \\\\',
'\\cline{4-8}','\\midrule\n','\\bottomrule'}
print(out.x, file = "C:\\file\\path\\file.tex", add.to.row = addtorow, 
      include.rownames = FALSE, booktabs = FALSE, hline.after=NULL,
      sanitize.text.function = function(x){x}, format.args=list(big.mark=" "), 
      floating.environment = 'sidewaystable') 
```

One final plot of the fitted values
from this additive model in Figure \@ref(fig:Figure4-16) hopefully crystalizes
the implications
...
```{r Figure4-16,fig.cap="(ref:fig4-16)",echo=F,warning=F,message=F}
par(mfrow=c(1,2))
intplot(jitter(fitted(debt1r))~cigbuy*incomegp,data=debtc,col=c(1,3),
        lwd=2,main="(a)",ylab="Mean of ProDebt")
intplot(jitter(fitted(debt1r))~incomegp*cigbuy,data=debtc,lwd=2,
        main="(b)",ylab="Mean of ProDebt")
```
